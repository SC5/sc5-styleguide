...
module.exports.applyStyles = function() {
  ...
  function bufferFileContents(file, enc, done) {
    ...
    if (path.extname(file.path) === '.css') {
      fileBuffer += file.contents.toString();
    }
  ...
  return through(throughOpts, bufferFileContents, function(cb) {
    ...
    // Combine preprocessed styles to styleguide.css
    _this.push(new File({
      path: 'styleguide.css',
    ...
    pseudoStylesPromise = Q.Promise(function(resolve) {
      var result = pseudoSelectors.stylesFromString(fileBuffer);
      _this.push(new File({
    ...
    atRulesPromise = Q.Promise(function(resolve) {
      var result = atRules.stylesFromString(fileBuffer);
      _this.push(new File({
    ...
    Q.all([pseudoStylesPromise, atRulesPromise]).then(function() {
      if (socketIsOpen()) {
        serverInstance.io.emitStylesChanges();
      }
      cb();
    });
  });
