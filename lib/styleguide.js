...
module.exports.generate = function(options) {
  ...
  return through(throughOpts, bufferFileContents, function(callback) {
    var _this = this,
    generateStylesFile,
    pushStylesFileToStream;

    generateStylesFile = Q.promise(function(resolve) {
      var srcFiles;
      srcFiles = [distPath + '/css/_fontpath_and_mixin_definition.css'];

      if (sgOptions.includeDefaultStyles) {
    ...
    pushStylesFileToStream = Q.promise(function(resolve) {
      Q.all([generateStylesFile]).then(function() {
        try {

          gulp.src(distPath + '/css/styleguide-app.css')
          .pipe(through.obj(function(file, enc, cb) {
            if (sgOptions.parsers.css === 'postcss') {
              filesBuffer[file.path] = file.contents.toString('utf8');
            }
          ...
          console.log('ERROR: styleguide-app.css does not exist.', e);
        }
      });
    });

    Q.all([pushStylesFileToStream]).then(function() {
