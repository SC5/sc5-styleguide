...
var through = require('through2'),
  ...
  scopedStyles = require(__dirname + '/modules/scoped-styles'),
          ...
          preprocess.getStream(Object.keys(filesBuffer), opt, reject)
            .pipe(through.obj(function(file, enc, cb) {
              ...
              var styles = file.contents.toString(),
                  pseudoStyles = pseudoSelectors.stylesFromString(styles);

              // Create stylesheet that contains pseudo styles
              _this.push(new File({
                path: 'styleguide_pseudo_styles.css',
                contents: new Buffer(pseudoStyles)
              }));

              _this.push(file);

              Q.all([
                atRules.stylesFromString(styles),
                scopedStyles.stylesFromString(styles),
                scopedStyles.stylesFromString(pseudoStyles)
              ]).spread(function(atRulesCss, scopedStylesCss, scopedPseudoStylesCss) {
                _this.push(new File({
                  path: 'styleguide_at_rules.css',
                  contents: new Buffer(atRulesCss)
                }));
                _this.push(new File({
                  path: 'scoped_styleguide.css',
                  contents: new Buffer(scopedStylesCss)
                }));
                _this.push(new File({
                  path: 'scoped_styleguide_pseudo_styles.css',
                  contents: new Buffer(scopedPseudoStylesCss)
                }));
                cb();
              });
