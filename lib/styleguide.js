  ...
  kss.parse(files, options, function(err, styleguide) {
    if (err) {
      ...
      json.sections.map(function(section) {
        if (section.markup) {
          var re = /^([\s\S]*)<sg\:wrapper>([\s\S]*)<\/sg\:wrapper>/g,
            match = re.exec(section.markup);
          if (match) {
            section.markup = match[1];
            section.wrappedMarkup = match[2].replace('<sg:wrapper-content/>', section.markup);
            section.modifiers.forEach(function(modifier){
              modifier.wrappedMarkup = match[2].replace('<sg:wrapper-content/>', modifier.markup);
            });
          }
        }
        return section;
