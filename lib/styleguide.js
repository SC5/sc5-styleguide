...
function replaceVariableFilePathWithHash(json) {
  var map = {};
  if (json && json.variables) {
    json.variables.forEach(function(variable) {
      map[variable.fileHash] = variable.file;
      variable.file = relativeToRoot(variable.file);
    });
  }
  return map;
}
...
function relativeToRoot(p) {
  return path.relative(__dirname, p).replace(/\.\.\/|\.\.\\/ig, '');
}
...
module.exports = function(options) {
  ...
  return through(throughOpts, bufferFileContents, function(callback) {
      ...
      Q.all([parseKSSPromise, parseVariablesPromise]).spread(function(sections, variables) {
        ...
        variableFileHashes = replaceVariableFilePathWithHash(styleguide);
        ...
        Q.all([overviewProcessed, preProcessingDone, filesCopied, indexHtmlProcessed])
          .then(function() {
            if (opt.server) {
              opt.variableFileMap = variableFileHashes;
              startServer(opt);
            }
            emitCompileSuccess();
          })
