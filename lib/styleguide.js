...
function copyUsedOptionsToJsonConfig(opt, json) {
  var used = ['appRoot', 'extraHead', 'beforeBody', 'afterBody', 'commonClass', 'title', 'disableEncapsulation', 'disableHtml5Mode', 'readOnly', 'sideNav', 'afterSections', 'showMarkupSection', 'favIcon', 'additionalNgDependencies'];
  json.config = {};
...
module.exports.generate = function(options) {
  ...
  return through(throughOpts, bufferFileContents, function(callback) {
      ...
      Q.all([parseKSSPromise, parseVariablesPromise]).spread(function(sections, variables) {
        ...
        Q.all([stylesCompiled, overviewProcessed, filesCopied, indexHtmlProcessed, favIcon])
          .then(function() {
            if (opt.server) {
