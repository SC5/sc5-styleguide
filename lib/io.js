...
module.exports = function(io, sourcePath, outputPath) {
  ...
  loadVariables = function(socket) {
    fs.readFile(sourcePath + '/styles/_styleguide_variables.scss', {encoding: 'utf8'}, function(err, data) {
      ...
      socket.emit('variables-from-server', JSON.stringify(values));
  ...
  saveVariables = function(socket) {
    return function(data) {
      ...
      sassVariables = JSON.parse(data.sassVariables);
      ...
      // TODO fix path to be consistent with source
      fs.writeFile(outputPath + '/sass/_styleguide_variables.scss', lines.join('\n'), function(err, data) {
        if (err) {
          return console.error(err);
        }
