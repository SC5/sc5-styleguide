...
angular.module('sgApp')
  .controller('MainCtrl', function ($scope, $location, $state, $rootScope,
    ...
    $rootScope.saveVariables = function() {
      ...
      if (saveToServer) {
        socket.emit('variables-to-server', {
          sassVariables: JSON.stringify(Variables.getValues())
        });
        console.log('EVENT: variables-to-server');
      } else {
        localStorage.setItem('variables', JSON.stringify(Variables.getValues()));
      }
    };
    ...
    loadVariables = function(vars) {
      return function(data) {
        vars.setValues(JSON.parse(data));
        localStorage.setItem('variables', JSON.stringify(Variables.getValues()));
      };
    };
    ...
    if (io) {
      socket = io('/');
      socket.on('variables-from-server', loadVariables(Variables));
