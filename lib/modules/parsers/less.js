...
function parseVariableDeclarations(string) {
  ...
  ast.traverseByType('declaration', function(decl) {
    ...
    var varName,
        ...
        varVal = '',
        varLine = 1;
    decl.traverseByType('variable', function(node) {
      varName = node.content.toString();
      varFullName = node.toString();
      decl.traverseByType('value', function(node) {
        varVal = node.toString();
        varLine = node.start.line;
      });
