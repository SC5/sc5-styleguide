...
module.exports = function(ioServer, options) {
  ...
  var io = ioServer,
    compileError = false,
    variableFileMap = options.variableFileMap;

  if (options.styleVariables && !fs.existsSync(options.styleVariables)) {
    console.error('modules/io: Could not find SASS variables file', options.styleVariables);
    return;
  }
  ...
  function saveVariables(variables) {
    ...
    Object.keys(groupedVariables).forEach(function(fileHash) {
      filePromises.push(Q.promise(function(resolveFile) {
        var filePath = variableFileMap[fileHash],
